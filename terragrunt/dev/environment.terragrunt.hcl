locals {
  ...
  transfer_users = {
    test = {
      user_name       = "test"
      home_directory  = "test"
      ssh_key         = local.ssh_key # This is a ssh key owned by IBI team to test client connection. A customer ssh key will be manually added via the console
    }
  }
  ...
  tgw_id_backbone              = "tgw-0f286031caf843cb9"
  ...
  route53_resolver_rules = {
    ...
    #   rule_id = "rslvr-rr-6cefcla34e264c61b"
    # }
    dev_shared_services_emis-web_com = {
      rule_id = "rslvr-rr-4b6ebb37adf24a43b"
    }
    # dev_jsy_emis_web_com = {
  ...
  ### KMS ###
  key_users = [
    "arn:aws:iam::${local.account_id}:role/aws-reserved/sso.amazonaws.com/${local.region}/AWSReservedSSO_generic-dev-access_2353806602eb3e234",
  ...
  wsus_qsconfig_id_rings      = "7d4d0"
  wsus_qsconfig_id_ringscan   = "3e43"
  wsus_policy_scan_only_name  = "ibi_ewi_pol_scan_only"

  ### DNS ###
  delinos_cidr_block   = ["100.68.4.0/25"]
  ...
  bastion_sg_rules_cidr_blocks = {
    rule1 = { type = "ingress", from = 3389, to = 3389, protocol = "tcp", cidr = local.delinos_cidr_block,     desc = "Allow RDP in from Delinos Distributed Engine CIDR" }
  }
  ...
  sql_sg_rules_cidr_blocks = {
    ...
    rule7 = { type = "egress",  from = 1433, to = 1433, protocol = "tcp", cidr = local.ss_sql_subnet_cidr,    desc = "Allow 1433 outbound to shared services sql subnet" }
  ...
  standard_sg_rules_cidr_blocks = {
    ...
    rule4 = { type = "ingress", from = 3389, to = 3389, protocol = "tcp", cidr = "${local.delinos_cidr_block}",               desc = "Allow RDP in from Delinos Distributed" }
    ...
    rule26 = { type = "ingress", from = 5985, to = 5986, protocol = "tcp", cidr = local.ss_ad_cidr,                          desc = "allow 135 inbound from shared services" }
