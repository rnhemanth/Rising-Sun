...
module "ec2" {
  ...
  root_kms_key_id             = each.value.server_function == "ibi-mgmt" ? module.kms_app.key_arn : module.kms.key_arn
  default_ebs_kms_key_id      = each.value.server_function == "ibi-mgmt" ? module.kms_app.key_arn : module.kms.key_arn
  ebs_kms_key_id              = each.value.server_function == "ibi-mgmt" ? module.kms_app.key_arn : module.kms.key_arn
  ...
  vpc_security_group_ids = each.value.server_function == "ibi-mgmt" ? [
  ...
  ebs_block_device = [
    for device in [
      each.value.server_function != "ibi-mgmt" ? {
        device_name = "/dev/sdb"
        volume_type = "gp3"
      ...
      each.value.server_function == "ibi-ssis" ? {
        device_name = "/dev/sdc"
        volume_type = "gp3"
      ...
      each.value.server_function == "ibi-mgmt" ? {
        device_name = "/dev/sde"
        volume_type = "gp3"
        volume_size = coalesce(each.value.e_volume_size, 20)
        throughput  = coalesce(each.value.e_throughput, 125)
        iops        = coalesce(each.value.e_iops, 3000)
        encrypted   = true
        tags = {
          DriveName   = "DATA"
          DriveLetter = "E"
      ...
      each.value.server_function == "ibi_ssis" ? {
        device_name = "/dev/sdf"
        volume_type = "gp3"
  ...
  tags = {
    ...
    lifecyle                  = coalesce(each.value.lifecycle_tag, "production")
    rg_service                = coalesce(each.value.rg, "aws-ibi")
  }
