...
variable "transit_gateway_attachments" {
  ...
  type = map(object({
    ...
    # Whether Appliance Mode support is enabled. If enabled, a traffic flow between a source and destination uses the same AvailabilityZone for the VPC attachment for the lifetime of that flow. Valid values: `disable`, `enable`. Default value: `disable`.
...
variable "name" {
  ...
  description = "environment, service and identifier, part of naming standard"
}
...
variable "wsus_rg_prefix" {
  type        = string
  description = "resource group prefix i.e eng ec"
  default     = "ibi_expl"
}
...
variable "ibi_sg_rules_cidr_blocks" {
  type = map(object({
    desc     = string
    type     = string
    from     = number
  ...
  description = "Security group rules to CIDR blocks - ibi sql"
  default     = {}
}
...
variable "sql_sg_rules_self" {
  ...
  description = "Security group rules to self reference - ibi sql."
  default     = {}
}