...
# resource "aws_resourcegroups_group" "ring2" {
#   name = "${lower(var.wsus_rg_prefix)}_rg_wsus_ring_2"
#   description = "Contains EC2 instances with a wsus tag of ring_2"
...
# resource "aws_resourcegroups_group" "ring3" {
#   name = "${lower(var.wsus_rg_prefix)}_rg_wsus_ring_3"
#   description = "Contains EC2 instances with a wsus tag of ring_3"
...
# resource "aws_resourcegroups_group" "ring4" {
#   name = "${lower(var.wsus_rg_prefix)}_rg_wsus_ring_4"
#   description = "Contains EC2 instances with a wsus tag of ring_4"
...
# resource "aws_resourcegroups_group" "holding2" {
#   name = "${lower(var.wsus_rg_prefix)}_rg_wsus_holding_ring_2"
#   description = "Contains EC2 instances with a wsus tag of holding_ring_2"
...
resource "aws_ssm_document" "wsus_install_updates" {
  name          = "EMIS-WSUSInstallUpdates"
  document_type = "Command"
  content = jsonencode({
    schemaVersion = "2.2"
    description   = "WSUS install updates and reboot | Manual execution of WSUS patches to EC2 instances."
...
resource "aws_ssm_document" "wsus_install_updates_noreboot" {
  name          = "EMIS-WSUSInstallUpdatesNoReboot"
  document_type = "Command"
  content = jsonencode({
    schemaVersion = "2.2"
    description   = "WSUS install updates - No reboot | Manual execution of WSUS patches to EC2 instances, without any reboot."
...
resource "aws_ssm_document" "wsus_checkupdates" {
  name          = "EMIS-WSUSCheckForUpdates"
  document_type = "Command"
  content = jsonencode({
    schemaVersion = "2.2"
    description   = "WSUS check for updates | Performs a WSUS scan on EC2 instances for approved updates within the WSUS catalog."
...
resource "aws_ssm_document" "windows_wsus_settings" {
  name          = "EMIS-WSUSConfigureClientSettings"
  document_type = "Command"
  content = jsonencode({
    schemaVersion = "2.2"
    description   = "Configure WSUS client | Sets the required WSUS configuration on client EC2 instances."
    mainSteps = [
      {
        ...
        inputs = {
          runCommand = [
            ...
            "Write-Output \"EC2 is in $az\"",
            "if ((Get-WmiObject win32_computersystem).partofdomain -eq $false)  {",
            "Write-Output \"EC2 in not a member of a Domain $DomainOutput\"",
            "if ($az -eq 'eu-west-2a') {$wsusserverips = '{{WsusDownstreamA}}' } else {$wsusserverips = '{{WsusDownstreamB}}'}",
            "Write-Output \"WSUS server is $wsusserverips\"}",
            "else {$DomainOutput = (Get-WmiObject win32_computersystem).Domain",
            "Write-Output \"EC2 is a member of Domain $DomainOutput\"",
            "if ($az -eq 'eu-west-2a') {$wsusips = '{{WsusDownstreamA}}' } else {$wsusips = '{{WsusDownstreamB}}'}",
            "Write-Output \"WSUS server is $wsusips\"",
            "$wsusserverips = $wsusips",
            "Write-Output \"WSUS server is $wsusserverips\"}",
            "$wsusprotocol = 'http://'",
            "$wsusport = ':8530'",
            ...
            "$WUServer = Get-ItemPropertyValue 'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows\\WindowsUpdate' -Name WUServer",
            "Set-ItemProperty -Path 'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows\\WindowsUpdate' -Name WUStatusServer -Value $wsusserver -Force",
            "$WUStatusServer = Get-ItemPropertyValue 'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows\\WindowsUpdate' -Name WUStatusServer",
            ...
            "Write-Output 'Disabling group targeting.'",
            "Set-ItemProperty -Path 'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows\\WindowsUpdate' -Name TargetGroupEnabled -Value 0 -Force",
            "$TargetGroupEnabled = Get-ItemPropertyValue 'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows\\WindowsUpdate' -Name TargetGroupEnabled",
            ...
            "$DisableDualScan = Get-ItemPropertyValue -Path 'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows\\WindowsUpdate' -Name DisableDualScan",
            ...
            "$ActiveHours = Get-ItemPropertyValue -Path 'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows\\WindowsUpdate' -Name ActiveHours",
            "Write-Output \"WUServer registry value: $WUServer\"",
            "Write-Output \"UseWUServer registry value: $UseWUServer\"",
            ...
            "Write-Output 'Restarting wuauserv service.'",
...
resource "aws_ssm_document" "wsus_tag" {
  name          = "EMIS-WSUSUpdateTagAssignment"
  document_type = "Command"
  content = jsonencode({
    schemaVersion = "2.2"
    description   = "Update 'wsus' tag | Updates 'wsus' tag to switch between live patching and paused patching. Use 'holding_ring_x' to pause patching."
...
module "s3_logs_bucket" {
  ...
  source = "git::https://github.com/emisgroup/terraform-aws-s3.git?ref=v0.1.0"

  bucket = "${var.environment}-ssm-plat-s3-wsus-logs-${data.aws_caller_identity.current.account_id}"
