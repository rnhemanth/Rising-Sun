...
resource "aws_iam_policy" "ParameterStoreGitHubDeployerPolicy" {
  name = "${local.project_name}-sec-pol-${var.identifier}ParameterStoreGitHubDeployerPolicy"
  policy = jsonencode({
    Statement = [
      {
        Effect = "Allow"
...
resource "aws_iam_policy" "TransferGitHubDeployerPolicy" {
  name = "${local.project_name}-sec-pol-${var.identifier}TransferGitHubDeployerPolicy"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_policy" "R53GitHubDeployerPolicy" {
  name = "${local.project_name}-sec-pol-${var.identifier}R53GitHubDeployerPolicy"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_role_policy_attachment" "TransferGitHubDeployerPolicy-attach" {
  role = aws_iam_role.github_deployer.name
  policy_arn = aws_iam_policy.TransferGitHubDeployerPolicy.arn
}
